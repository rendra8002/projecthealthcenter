                ...
                <div class="footer-thumb">
                    ...
                    <ul class="list-unstyled">
                        @foreach ($mediasocials as $media)
                            <li class="media mb-3">
                                <img src="{{ $media->photo ? asset('storage/' . $media->photo) : asset('assets/images/default-icon.png') }}"
                                    class="mr-3 rounded-circle img-circle" width="40" height="40"
                                    alt="{{ $media->name_mediasocial }}">
                                <div class="media-body">
                                    <a href="{{ $media->link }}" target="_blank">
                                        <strong>{{ $media->name_account }}</strong>
                                    </a><br>
                                    <small class="text-muted">{{ $media->name_mediasocial }}</small>
                                </div>
                            </li>
                        @endforeach
                    </ul>
